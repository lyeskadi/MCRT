% Run MCRT.m and plot results (for multiple nground)
% Run from halo/1031 directory
close all
clear all
%% General params
L_RAID = 1.5;
...
useOpacity = 1;
Nparticles = 1e6;
nground = [3e18 1e19 3e19 1e20 3e20];

for j = 1:length(nground)
% Run MC for each half separately (like OES analysis)
[emission_radius, escapeproba] = MCRT(nground(j),branching_ratio,TSfile,Nparticles,'half',useOpacity);
[emission_radius_neg, escapeproba_neg] = MCRT(nground(j),branching_ratio,TSfile,Nparticles,'neghalf',useOpacity);
...
radialprofile_RT{j} = [flip(radialprofile_halo_neg) radialprofile_halo];
radialprofile_init = [flip(radialprofile_init_neg) radialprofile_init];
end
%% Plot radial profile of emission line

% cmap = 'lajolla';
% load(['/home/lyes/Documents/MATLAB/colormaps/' cmap '/' cmap '.mat'])
% ncolors = length(nground);
% 
% for j = 1:ncolors
%     color_RT{j} = lajolla(floor(j*256/(ncolors)),:);    
% end
% Define colors
...
rosso = [0.83 0 0];
persimmon = [0.93 0.35 0]; % Dark orange
amber = [1.0, 0.75, 0.0]; % Light orange
schoolbus = [1.0, 0.85, 0.0]; % Bright yellow
titaniumyellow = [0.93, 0.9, 0.0];
color_init = 0.2*[1 1 1];
color_RT = {titaniumyellow,schoolbus,amber,persimmon,rosso};
...
hold on
plot(rplot,radialprofile_init,'Linewidth',1.5,'Color',color_init)
%plot(rplot,radialprofile_halo)
legendtext = {'OES 502\,nm','CoRa 502\,nm'};
for j=1:length(nground)
plot(rplot,radialprofile_RT{j},'Linewidth',2,'Color',color_RT{j})
legendtext{j+2} = string(['$n_1=\,$' num2str(nground(j))]);
end
hold off
set(gcf,'color','w')
set(groot,'defaultAxesTickLabelInterpreter','latex');
set(gca,'FontSize',13) %15
xlim([-100,100])
ylim([0, 1.3])

legend('OES 502\,nm','Initial positions',legendtext,'Fontsize',15,'Interpreter','latex')
ax=gca;
% for j=1:length(nground)
% ax.Legend.String{j+2} = ;
% end
xlabel('Radial position (mm)','Fontsize',17,'Interpreter','latex')
ylabel('Intensity (arb)','Fontsize',17,'Interpreter','latex')
